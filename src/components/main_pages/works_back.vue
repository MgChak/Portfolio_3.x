<template>
    ...
    <div class="works_container" :style="[background_color,works_container_size]">
        ...
        <div class="view_window" :style="view_window_size">
            ...
            <div class="list_conatiner" :style="{left:slides_position, transition:slides_transition}" >
...
const store = useStore()
    ...
    let slides_position = computed(()=>{
        if (!store.is_touch_slidshow){
            return `${store.get_slides_position}px`
        }else{
            return `${store.slideshow_track_touch_position}px`
        }
    ...
    let animation_queue_click_pagemove = (val)=>{
        ...
        setTimeout(()=>{
            ...
            //卡片沉降到-3
            store.z_index_page_number = store.page_on
        },store.slide_show_animation_delay*310)
        //0.6s后
        setTimeout(()=>{
            //修改显示的信息 
        ...
        },store.slide_show_animation_delay*300+310)
    }
    //翻页
    let slides_move = (tar)=>{
